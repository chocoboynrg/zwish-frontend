...
import { formatParticipantStatus, formatRoleLabel } from './event-ui.utils';
...
@Component({
  ...
  template: `
    ...
    @if (show) {
      <section class="request-modal participants-modal">
        <div class="request-modal-card participants-modal-card" (click)="$event.stopPropagation()">
          ...
          @if (participantsLoading) {
            <div class="state-card compact-state">Chargement participants...</div>
          }
          @if (participantsError) {
            <div class="state-card error compact-state">
              {{ participantsError }}
            </div>
          }
          ...
          @if (participants && participants.participants.length > 0) {
            <div class="participant-list">
              @for (p of participants.participants; track p.id) {
