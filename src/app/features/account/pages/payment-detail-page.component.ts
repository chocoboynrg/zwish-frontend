...
@Component({
          ...
          @if (payment && !loading) {
            <div class="hero-content">
              <div class="hero-left">
                ...
                <div class="hero-item-name">
                  {{ payment.contribution?.wishlistItem?.title ?? 'Transaction' }}
                </div>
                ...
                <div class="hero-meta-pills">
                  ...
                  @if (payment.paymentMethod) {
                    <span class="hero-meta-pill">{{ payment.paymentMethod }}</span>
                  }
                </div>
              </div>
            ...
            @if (payment.status === 'INITIATED' && payment.paymentUrl) {
              <div class="hero-action-bar">
                ...
                @if (isMockPayment()) {
                  <button class="btn-mock-succeed" (click)="simulateSuccess()" [disabled]="simulating">
                    @if (simulating) {
                      ⏳ Simulation...
                    } @else {
                ...
                <div class="timeline-item" [class.done]="true">
                  ...
                  <div class="tl-content">
                    ...
                    <div class="tl-date">
                      {{
                        payment.initiatedAt
                          ? (payment.initiatedAt | date: 'dd MMM yyyy HH:mm')
                ...
                <div class="timeline-item" [class.done]="payment.status === 'SUCCEEDED'">
                  ...
                  <div class="tl-content">
                    ...
                    @if (payment.confirmedAt) {
                      <div class="tl-date">
                        {{ payment.confirmedAt | date: 'dd MMM yyyy HH:mm' }}
                      </div>
                    }
                    @if (payment.failedAt) {
                      <div class="tl-date">{{ payment.failedAt | date: 'dd MMM yyyy HH:mm' }}</div>
                    }
                    @if (!payment.confirmedAt && !payment.failedAt) {
                      <div class="tl-date tl-muted">En attente...</div>
                    }
                  </div>
                </div>
                @if (payment.refundedAt) {
                  ...
                  <div class="timeline-item done">
                    ...
                    <div class="tl-content">
                      ...
                      <div class="tl-date">
                        {{ payment.refundedAt | date: 'dd MMM yyyy HH:mm' }}
                      </div>
                    </div>
                  </div>
                }
                ...
                <div class="info-card">
                  ...
                  <div class="info-list">
                    ...
                    <div class="info-row">
                      ...
                      <span
                        class="status-badge"
                        [ngClass]="getContribStatusClass(payment.contribution!.status)"
                      >
                    ...
                    @if (payment.contribution!.wishlistItem) {
                      <div class="info-row">
                  ...
                  <!-- Lien vers événement -->
                  @if (payment.contribution!.event?.id) {
                    <a
                ...
                <div class="pending-banner-left">
                  ...
                  <div>
                    ...
                    <div class="pending-banner-desc">
                      Votre session de paiement est active. Cliquez sur le bouton pour finaliser
                      votre transaction.
                    </div>
                  </div>
                </div>
  ...
  styles: [
    `
      ...
      .page-hero.hero-succeeded {
        background: linear-gradient(135deg, #000 60%, #064e3b);
      }
      .page-hero.hero-initiated,
      .page-hero.hero-pending {
        background: linear-gradient(135deg, #000 60%, #78350f);
      }
      .page-hero.hero-failed,
      .page-hero.hero-expired {
        background: linear-gradient(135deg, #000 60%, #7f1d1d);
      }
      .page-hero.hero-refunded {
        background: linear-gradient(135deg, #000 60%, #4c1d95);
      }
      ...
      .btn-mock-succeed:hover:not(:disabled) {
...
export class PaymentDetailPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.paymentId || Number.isNaN(this.paymentId)) {
  ...
  private loadPayment(): void {
    this.myPaymentsService.getOne(this.paymentId).subscribe({
  ...
  isMockPayment(): boolean {
    return this.payment?.status === 'INITIATED' && this.payment?.provider === 'OTHER';
  }

  simulateSuccess(): void {
    if (!this.payment || this.simulating) return;
    this.simulating = true;
    this.myPaymentsService.mockSucceed(this.payment.id).subscribe({
      next: (p) => { this.payment = p; this.simulating = false; },
      error: (e: any) => {
  ...
  getHeroClass(): string {
    ...
    const m: Record<string, string> = {
      SUCCEEDED: 'hero-succeeded',
      INITIATED: 'hero-initiated',
      PENDING: 'hero-pending',
      FAILED: 'hero-failed',
      EXPIRED: 'hero-expired',
      REFUNDED: 'hero-refunded',
    };
  ...
  getStatusPillClass(s: string): string {
    const m: Record<string, string> = {
      SUCCEEDED: 'hero-status-pill pill-succeeded',
      INITIATED: 'hero-status-pill pill-initiated',
      PENDING: 'hero-status-pill pill-pending',
      FAILED: 'hero-status-pill pill-failed',
      EXPIRED: 'hero-status-pill pill-expired',
      REFUNDED: 'hero-status-pill pill-refunded',
    };
  ...
  getContribStatusClass(s: string): string {
    const m: Record<string, string> = {
      CONFIRMED: 'status-badge cs-confirmed',
      AWAITING_PAYMENT: 'status-badge cs-awaiting',
      FAILED: 'status-badge cs-failed',
      CANCELLED: 'status-badge cs-cancelled',
      REFUNDED: 'status-badge cs-refunded',
    };
