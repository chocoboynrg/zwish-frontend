...
@Component({
  ...
  template: `
    <div class="page-wrap">
      ...
      <div class="page-body">
        ...
        @if (!loading() && mainTab() === 'joined') {
          ...
          @if (joinedEvents().length > 0) {
            <div class="joined-grid">
              @for (e of joinedEvents(); track e.id) {
                <a
                ...
                >
                  ...
                  <div
                    class="jcard-strip"
                    [class.strip-soon]="isSoon(e.eventDate)"
                    [class.strip-past]="isPast(e.eventDate)"
                  ></div>
                  <div class="jcard-body">
                    ...
                    <div class="jcard-head">
                      ...
                      <span class="jcard-tag" [ngClass]="getEventTagClass(e.eventDate)">{{
                        getEventTagLabel(e.eventDate)
                      }}</span>
                    </div>
                    <!-- Ma contribution -->
                    @if (getContrib(e.id); as c) {
                      <div class="jcard-contrib">
                        ...
                        <div class="jcard-contrib-status">
                          <span class="contrib-badge" [ngClass]="getContribClass(c.status)">{{
                            getContribLabel(c.status)
                          }}</span>
        ...
        <!-- ONGLET : J'ORGANISE     -->
        <!-- ─────────────────────── -->
        @if (!loading() && mainTab() === 'organized') {
          <!-- Sous-onglets -->
          <div class="org-tabs">
            <button
              class="org-tab"
              [class.active]="orgTab() === 'upcoming'"
              (click)="orgTab.set('upcoming')"
            >
              À venir
              @if (upcomingOrg().length) {
                <span class="org-tab-count">{{ upcomingOrg().length }}</span>
              }
            </button>
            <button
              class="org-tab"
              [class.active]="orgTab() === 'past'"
              (click)="orgTab.set('past')"
            >
              Passés
              @if (pastOrg().length) {
                <span class="org-tab-count">{{ pastOrg().length }}</span>
              }
            </button>
            <button
              class="org-tab"
              [class.active]="orgTab() === 'archived'"
              (click)="orgTab.set('archived')"
            >
              Archivés
              @if (archivedOrg().length) {
                <span class="org-tab-count">{{ archivedOrg().length }}</span>
              }
            </button>
          </div>
          <!-- Empty org -->
          @if (currentOrg().length === 0) {
            <div class="empty-block">
              <div class="empty-icon">
                {{ orgTab() === 'upcoming' ? '🎉' : orgTab() === 'past' ? '📅' : '📦' }}
              </div>
              <div class="empty-title">{{ orgEmptyTitle() }}</div>
              <p>{{ orgEmptyDesc() }}</p>
              @if (orgTab() === 'upcoming') {
                <a routerLink="/app/events/new" class="btn-yellow">Créer un événement</a>
              }
            </div>
          }
          <!-- Grille org -->
          @if (currentOrg().length > 0) {
            <div class="org-grid">
              @for (e of currentOrg(); track e.id) {
                <a
                  [routerLink]="['/app/events', e.id]"
                  class="org-card"
                ...
                >
                  <div class="org-card-cover">
                    ...
                    <span class="org-cover-pill" [ngClass]="getCoverPillClass(e.eventDate)">{{
                      getCoverPillLabel(e.eventDate)
                    }}</span>
                  </div>
                  ...
                  <div class="org-card-footer">
                    <span class="org-card-cta"
                      >{{ orgTab() === 'archived' ? 'Voir' : 'Gérer' }} →</span
                    >
                  </div>
                </a>
              }
  ...
  styles: [
    `
      ...
      .jcard-contrib-amount {
        font-size: 1rem;
        font-weight: 900;
        color: #111;
      }
      .jcard-contrib-status {
        display: flex;
        align-items: center;
        gap: 8px;
        flex-wrap: wrap;
      ...
      .org-card:nth-child(3n + 2) .org-card-cover {
        background: linear-gradient(135deg, #0d1117, #1c3a4a);
      }
      .org-card:nth-child(3n + 3) .org-card-cover {
...
export class MyEventsPageComponent implements OnInit {
  ...
  readonly orgEmptyDesc = computed(() => {
    if (this.orgTab() === 'upcoming')
      return 'Créez votre premier événement et partagez votre wishlist.';
  ...
  getContribClass(s: string): string {
    const m: Record<string, string> = {
      CONFIRMED: 'contrib-badge cb-confirmed',
      AWAITING_PAYMENT: 'contrib-badge cb-awaiting',
      FAILED: 'contrib-badge cb-failed',
    };
