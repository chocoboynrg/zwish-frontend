...
@Component({
  ...
  template: `
    <div class="page">
      ...
      @if (payment(); as p) {
        ...
        <div class="payment-header">
          ...
          <div class="header-actions">
            ...
            @if (p.status === 'SUCCEEDED') {
              <button class="btn-warn" (click)="showRefundModal = true">Rembourser</button>
            }
          </div>
        </div>
        <div class="grid-layout">
          ...
          <div class="card">
            ...
            <div class="info-list">
              <div class="info-row">
                <span>Méthode</span><span>{{ p.paymentMethod }}</span>
              </div>
              <div class="info-row">
                <span>Provider Ref</span><span class="mono">{{ p.providerReference ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span>Transaction ID</span
                ><span class="mono">{{ p.providerTransactionId ?? '—' }}</span>
              </div>
              <div class="info-row">
                <span>URL paiement</span>
                @if (p.paymentUrl) {
                  <a [href]="p.paymentUrl" target="_blank" class="link">Ouvrir →</a>
                }
                @if (!p.paymentUrl) {
          ...
          <div class="card">
            ...
            <div class="info-list">
              <div class="info-row">
                <span>Créé</span><span>{{ p.createdAt | date: 'dd/MM/yyyy HH:mm' }}</span>
              </div>
              <div class="info-row">
                <span>Initié</span
                ><span>{{ p.initiatedAt ? (p.initiatedAt | date: 'dd/MM/yyyy HH:mm') : '—' }}</span>
              </div>
              <div class="info-row">
                <span>Expire</span
                ><span>{{ p.expiresAt ? (p.expiresAt | date: 'dd/MM/yyyy HH:mm') : '—' }}</span>
              </div>
              @if (p.confirmedAt) {
                <div class="info-row">
                  <span>Confirmé</span
                  ><span class="text-green">{{ p.confirmedAt | date: 'dd/MM/yyyy HH:mm' }}</span>
                </div>
              }
              @if (p.failedAt) {
                <div class="info-row">
                  <span>Échoué</span
                  ><span class="text-red">{{ p.failedAt | date: 'dd/MM/yyyy HH:mm' }}</span>
                </div>
              }
              @if (p.refundedAt) {
                <div class="info-row">
                  <span>Remboursé</span
                  ><span class="text-purple">{{ p.refundedAt | date: 'dd/MM/yyyy HH:mm' }}</span>
                </div>
              }
            </div>
          </div>
          ...
          @if (p.payer) {
            <div class="card">
              ...
              <div class="info-list">
                <div class="info-row">
                  <span>Nom</span><span>{{ p.payer.name }}</span>
                </div>
                <div class="info-row">
                  <span>Email</span><span>{{ p.payer.email }}</span>
                </div>
                <div class="info-row">
                  <span>Profil</span>
          ...
          @if (p.contribution) {
            <div class="card">
              ...
              <div class="info-list">
                <div class="info-row">
                  <span>ID</span><span>#{{ p.contribution.id }}</span>
                </div>
                <div class="info-row">
                  <span>Statut</span>
                  <span class="badge" [ngClass]="getStatusBadge(p.contribution.status)">{{
                    p.contribution.status
                  }}</span>
                </div>
                <div class="info-row">
                  <span>Montant</span
                  ><span>{{ p.contribution.amount | number }} {{ p.currencyCode }}</span>
                </div>
                @if (p.contribution.event) {
                  <div class="info-row">
                    <span>Événement</span>
                    <a [routerLink]="['/admin/events', p.contribution.event.id]" class="link"
                      >{{ p.contribution.event.title }} →</a
                    >
                  </div>
                }
                @if (p.contribution.wishlistItem) {
                  <div class="info-row">
            ...
            <div class="card full-width">
              ...
              <div class="checks-grid">
                <div
                  class="check-item"
                  [ngClass]="
                    p.checks.paymentSucceededMatchesContribution ? 'check-ok' : 'check-fail'
                  "
                >
                  <span class="check-icon">{{
                    p.checks.paymentSucceededMatchesContribution ? '✓' : '✕'
                  }}</span>
                  <span>Paiement réussi ↔ Contribution confirmée</span>
                </div>
                <div
                  class="check-item"
                  [ngClass]="p.checks.paymentExpiredConsistency ? 'check-ok' : 'check-fail'"
                >
                  <span class="check-icon">{{
                    p.checks.paymentExpiredConsistency ? '✓' : '✕'
                  }}</span>
                  <span>Cohérence expiration</span>
                </div>
                <div
                  class="check-item"
                  [ngClass]="p.checks.noOrphanSucceededPayment ? 'check-ok' : 'check-fail'"
                >
                  <span class="check-icon">{{
                    p.checks.noOrphanSucceededPayment ? '✓' : '✕'
                  }}</span>
                  <span>Pas de paiement orphelin</span>
                </div>
                <div
                  class="check-item"
                  [ngClass]="p.checks.noDuplicateSucceededWebhooks ? 'check-ok' : 'check-fail'"
                >
                  <span class="check-icon">{{
                    p.checks.noDuplicateSucceededWebhooks ? '✓' : '✕'
                  }}</span>
          ...
          <div class="card full-width">
                ...
                @if (webhooksResponse()) {
                  <p class="section-sub">
                    ...
                    <span class="text-green"
                      >{{ webhooksResponse()!.summary.succeededCount }} SUCCEEDED</span
                    >
                    /
                    <span class="text-red"
                      >{{ webhooksResponse()!.summary.failedCount }} FAILED</span
                    >
                  </p>
                }
              </div>
            ...
            @if (!webhooksLoading() && webhooks().length === 0) {
              <p class="empty-webhooks">Aucun webhook pour ce paiement.</p>
            }
                      ...
                      <tr>
                        ...
                        <td>
                          <span
                            class="badge"
                            [ngClass]="
                              wh.externalStatus === 'SUCCEEDED' ? 'badge-green' : 'badge-red'
                            "
                          >
                            {{ wh.externalStatus }}
                          </span>
                        ...
                        <td>
                          <span
                            class="badge"
                            [ngClass]="getStatusBadge(wh.resultingPaymentStatus ?? '')"
                          >
                        ...
                        <td class="muted small">
                          {{ wh.processedAt ? (wh.processedAt | date: 'dd/MM/yy HH:mm') : '—' }}
                        </td>
            ...
            @if (webhooksTotalPages() > 1) {
              <div class="pagination">
                <button (click)="goToWebhookPage(whPage() - 1)" [disabled]="whPage() === 1">
                  ←
                </button>
                <span>Page {{ whPage() }} / {{ webhooksTotalPages() }}</span>
                <button
                  (click)="goToWebhookPage(whPage() + 1)"
                  [disabled]="whPage() === webhooksTotalPages()"
                >
                  →
                </button>
              </div>
      ...
      <!-- ===== MODAL VALIDER ===== -->
      @if (showSucceedModal) {
        <div class="modal-backdrop" (click)="showSucceedModal = false">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <label
              >Transaction ID (optionnel)<input
                type="text"
                [(ngModel)]="succeedTxId"
                placeholder="Ex: TXN_123456"
            /></label>
            <label
              >Référence provider (optionnel)<input
                type="text"
                [(ngModel)]="succeedRef"
                placeholder="Ex: REF_ABC"
            /></label>
            <label
              >Note interne (optionnel)<input
                type="text"
                [(ngModel)]="succeedNote"
                placeholder="Note pour l'audit"
            /></label>
      ...
      @if (showRefundModal) {
        <div class="modal-backdrop" (click)="showRefundModal = false">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <p class="modal-sub">
              {{ payment()?.amount | number }} {{ payment()?.currencyCode }} —
              {{ payment()?.payer?.name }}
            </p>
            <label
...
export class PaymentDetailAdminPageComponent implements OnInit {
  ...
  doRefund(): void {
    ...
    this.actionLoading.set(true);
    this.paymentsService
      .refund(p.id, this.refundReason.trim(), this.refundNote.trim() || undefined)
      .subscribe({
        next: (updated) => {
          this.payment.update((prev) => (prev ? { ...prev, ...updated } : prev));
          this.actionLoading.set(false);
