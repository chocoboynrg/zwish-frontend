...
@Component({
    ...
    <div class="page">
      ...
      <div class="page-header">
        <div>
          ...
          @if (response()) {
            <p class="subtitle">{{ response()!.summary.totalCount }} anomalie(s) détectée(s)</p>
          }
          @if (!response() && !loading()) {
            <p class="subtitle">Analyse des incohérences financières</p>
          }
        </div>
      ...
      <div class="issues-container">
        ...
        @if (!loading() && issues().length === 0 && response()) {
          <p class="empty-state">✅ Aucune anomalie détectée avec ces filtres.</p>
        }

        @if (issues().length > 0) {
          <div class="issues-list">
            @for (issue of issues(); track issue) {
              <div class="issue-card" [ngClass]="'issue-' + issue.severity">
                <!-- Header de l'issue -->
                ...
                <div class="issue-links">
                  ...
                  @if (issue.contributionId) {
                    <span class="contribution-ref"> Contribution #{{ issue.contributionId }} </span>
                  }
                </div>
                <!-- Détails techniques (expandable) -->
                @if (issue.details && hasDetails(issue.details)) {
  ...
  styles: [
    `
      ...
      .issue-details[open] summary::before {
        content: '▼';
      }
      .details-json {
        margin: 8px 0 0;
