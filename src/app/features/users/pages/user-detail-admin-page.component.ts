...
@Component({
  ...
  template: `
    <div class="page">
      ...
      @if (user(); as u) {
        ...
        <div class="profile-header">
          ...
          <div class="profile-info">
            ...
            <p>{{ u.email }}</p>
            <div class="badges">
              <span class="badge" [ngClass]="getRoleBadge(u.platformRole)">{{
                u.platformRole
              }}</span>
              <span class="badge" [ngClass]="u.isSuspended ? 'badge-red' : 'badge-green'">{{
                u.isSuspended ? 'Suspendu' : 'Actif'
              }}</span>
              <span class="badge" [ngClass]="u.emailVerifiedAt ? 'badge-green' : 'badge-gray'">{{
                u.emailVerifiedAt ? 'Email vérifié' : 'Non vérifié'
              }}</span>
            </div>
          </div>
          <div class="profile-actions">
            @if (!u.isSuspended && canSuspend(u)) {
              <button class="btn-warn" (click)="showSuspendModal = true">Suspendre</button>
            }
            @if (u.isSuspended && canSuspend(u)) {
              <button class="btn-success" (click)="doUnsuspend()" [disabled]="actionLoading()">
                {{ actionLoading() ? '...' : 'Réactiver' }}
              </button>
            }
            @if (isSuperAdmin() && u.platformRole !== 'SUPER_ADMIN' && u.id !== currentUserId()) {
              <button
                class="btn-role"
                (click)="
                  showRoleModal = true; newRole = u.platformRole === 'ADMIN' ? 'USER' : 'ADMIN'
                "
              >
                Changer rôle
              </button>
            ...
            <div class="info-grid">
              ...
              @if (u.isSuspended) {
                <div class="info-item">
                  ...
                  <span class="info-value warn">{{
                    u.suspendedAt | date: 'dd/MM/yyyy HH:mm'
                  }}</span>
                </div>
              }
              @if (u.isSuspended && u.suspensionReason) {
                <div class="info-item">
        ...
        @if (u.organizedEvents && u.organizedEvents.length > 0) {
          <div class="card">
            <h2>Événements organisés ({{ u.organizedEvents.length }})</h2>
            <div class="list">
              @for (e of u.organizedEvents; track e.id) {
                <div class="list-item">
        ...
        @if (u.latestContributions && u.latestContributions.length > 0) {
          <div class="card">
            ...
            <div class="table-wrapper">
              <table>
                ...
                <tbody>
                  @for (c of u.latestContributions; track c.id) {
                    <tr>
        ...
        @if (u.latestPayments && u.latestPayments.length > 0) {
          <div class="card">
            ...
            <div class="table-wrapper">
              <table>
                ...
                <tbody>
                  @for (p of u.latestPayments; track p.id) {
                    <tr>
                      <td>#{{ p.id }}</td>
        ...
        @if (u.latestReservations && u.latestReservations.length > 0) {
          <div class="card">
            <h2>Dernières réservations ({{ u.latestReservations.length }})</h2>
            <div class="list">
              @for (r of u.latestReservations; track r.id) {
                <div class="list-item">
        ...
        <!-- Audit log -->
        @if (u.latestAuditLogs && u.latestAuditLogs.length > 0) {
          <div class="card">
            <h2>Activité récente</h2>
            <div class="list">
              @for (log of u.latestAuditLogs; track log) {
                <div class="list-item">
      ...
      <!-- Modal suspension -->
      @if (showSuspendModal) {
        <div class="modal-backdrop" (click)="showSuspendModal = false">
      ...
      <!-- Modal rôle -->
      @if (showRoleModal) {
        <div class="modal-backdrop" (click)="showRoleModal = false">
          <div class="modal" (click)="$event.stopPropagation()">
