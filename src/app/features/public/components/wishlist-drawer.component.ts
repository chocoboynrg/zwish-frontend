...
@Component({
  ...
  template: `
    <!-- Overlay -->
    @if (drawer.isOpen()) {
      <div class="overlay" (click)="drawer.close()"></div>
    }
    ...
    <div class="drawer" [class.open]="drawer.isOpen()">
      ...
      <!-- Non connecté -->
      @if (!auth.isAuthenticated() && !drawer.loading()) {
      ...
      <!-- Vide -->
      @if (auth.isAuthenticated() && !drawer.loading() && drawer.events().length === 0) {
        <div class="drawer-empty">
          <div class="empty-icon">🎁</div>
      ...
      @if (auth.isAuthenticated() && !drawer.loading() && drawer.events().length > 0) {
        <div class="drawer-body">
          @for (event of drawer.events(); track event; let first = $first) {
            <div class="event-section" [class.recent]="event.eventId === drawer.lastAddedEventId()">
              <!-- Header événement -->
              <div class="event-head">
                <div class="event-head-left">
                  <div
                    class="event-dot"
                    [class.dot-recent]="event.eventId === drawer.lastAddedEventId()"
                  ></div>
                  <div>
                    <div class="event-title">
                      {{ event.eventTitle }}
                      @if (event.eventId === drawer.lastAddedEventId()) {
                ...
                <span class="item-count"
                  >{{ event.totalItems }} item{{ event.totalItems > 1 ? 's' : '' }}</span
                >
              </div>
              <!-- Items -->
              @if (event.items.length > 0) {
                <div class="items-list">
                  @for (item of event.items.slice(0, 5); track item) {
                    <div class="wish-item">
                      @if (item.imageUrl) {
                        <div class="wish-img">
                          <img
                      ...
                      @if (!item.imageUrl) {
                        <div class="wish-img wish-img-placeholder">🎁</div>
                      }
                      <div class="wish-info">
                        ...
                        <div class="wish-progress-wrap">
                          ...
                          <span
                            class="wish-pct"
                            [class.pct-funded]="item.fundingStatus === 'FUNDED'"
                          >
                        ...
                        <div class="wish-amounts">
                          ...
                          <span class="wish-target"
                            >{{ item.targetAmount | number: '1.0-0' }} XOF</span
                          >
                        </div>
                      </div>
                  ...
                  @if (event.items.length > 5) {
                    <div class="more-items">+ {{ event.items.length - 5 }} autre(s) item(s)</div>
                  }
                </div>
              }
              @if (event.items.length === 0) {
                <div class="items-empty">Aucun item dans cette wishlist.</div>
              }
      ...
      <!-- Footer -->
      @if (auth.isAuthenticated() && drawer.events().length > 0) {
